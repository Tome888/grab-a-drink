...
export default function Settings() {
  ...
  const handleSubmitLocation = async (e: React.FormEvent) => {
    ...
    await updateLocation(location.lat, location.lon, location.radius);
  };
  ...
  return (
    <>
      <div className="max-w-md mx-auto mt-10 p-6 bg-white dark:bg-gray-800 rounded shadow-md">
        ...
        {successCreds && <p className="text-green-500 mt-4">{successCreds}</p>}
      </div>
      ...
      <div className="max-w-md mx-auto mt-10 p-6 bg-white dark:bg-gray-800 rounded shadow-md">
        ...
        {locationLoading ? (
          ...
          <form onSubmit={handleSubmitLocation} className="flex flex-col gap-3">
            <input
              type="number"
              step="0.0001"
              value={location.lat}
            ...
            <input
              ...
              onChange={(e) => setLocation({ ...location, radius: parseFloat(e.target.value) })}
            ...
            {successLocation && <p className="text-green-500 mt-4">{successLocation}</p>}
          </form>
        )}
      </div>
    </>
