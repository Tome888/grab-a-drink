...
const resetUsers = db.transaction(() => {
  ...
  ).run(DEFAULT_USERNAME, DEFAULT_PASSWORD, DEFAULT_LAT, DEFAULT_LON, DEFAULT_RADIUS);
});
...
const initUserTable = () => {
  ...
  resetUsers();
  console.log("👍 User table initialized with default admin and location columns.");
};
...
const DEFAULT_MENU = [
  ...
  { name: "Long Island Iced Tea", price: 12, img_path: "/images/long_island.jpg" },
  { name: "Bloody Mary", price: 8, img_path: "/images/bloody_mary.jpg" },
  { name: "Sex on the Beach", price: 10, img_path: "/images/sex_on_the_beach.jpg" },
];

const resetMenu = db.transaction(() => {
  const insert = db.prepare("INSERT INTO menu (name, price, img_path) VALUES (?, ?, ?)");
  DEFAULT_MENU.forEach((item) => insert.run(item.name, item.price, item.img_path));
});

const initMenuTable = () => {
  db.prepare(`DROP TABLE IF EXISTS menu`).run();

  db.prepare(`CREATE TABLE menu (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    name TEXT,
    price REAL,
    img_path TEXT
